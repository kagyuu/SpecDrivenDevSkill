...
> `V0.8_testbed`(Skill V0.8のフルパイプライン実地検証)の起点となる要件定義書です。`V0.7_testbed`の起点要件定義書(CR-001適用前の原本)をそのまま流用し、以降のP002〜P302をV0.8のルールで新規に実行します。要件文書そのものはV0.8で変更していないため、V0.7_testbedとの差分がSkillのルール変更に起因するものだけになるようにしています。
...
| 表示 | 予約サマリ(予約者名・件名) | 予約済みセルに表示する簡易情報。参加予定人数は表示しない |
...
| 入力 | 備考 | 任意、最大500文字 |
| 入力 | オンライン会議URL | 任意、最大500文字。入力する場合は `http://` または `https://` で始まること(※CR-001により追加) |
| 表示 | 重複エラーメッセージ | 選択した会議室・時間帯が既に予約済みの場合に表示 |
| 表示 | 収容人数超過エラーメッセージ | 参加予定人数が選択した会議室の収容人数を超える場合に表示 |
| 表示 | オンライン会議URL形式エラーメッセージ | オンライン会議URLが `http://`/`https://` で始まらない、または500文字を超える場合に表示(※CR-001により追加) |

### S04 予約詳細・編集画面

| 種別 | 項目名 | 内容 |
| --- | --- | --- |
| 表示 | 予約内容(会議室/日時/件名/参加者/参加予定人数/備考/オンライン会議URL/予約者) | S03と同項目を表示。オンライン会議URLが設定されている場合はクリック可能なリンクとして表示する(※CR-001により追加。項目自体はS02カレンダー画面には表示しない) |
| 入力 | 編集用の各項目 | 予約者本人または管理者のみ編集可。参加予定人数・オンライン会議URLもS03と同じルールで編集可(※CR-001によりオンライン会議URLを追加) |
...
| GET | `/api/reservations/{reservation_id}` | 予約の詳細を取得する(オンライン会議URLを含む。※CR-001により追加) |
| POST | `/api/reservations` | 予約を新規登録する(重複チェックを行う。任意項目としてオンライン会議URLを受け付ける。※CR-001により追加) |
| PUT | `/api/reservations/{reservation_id}` | 予約内容を更新する(重複チェックを行う。オンライン会議URLの変更も可能。※CR-001により追加) |
