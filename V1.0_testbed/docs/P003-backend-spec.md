...
### 2.3 開発・テスト時のクロスオリジン方針

本システムはクライアント(Vite開発サーバ、既定 `http://localhost:5173`)とサーバ(uvicorn、既定 `http://localhost:8000`)を**別プロセス・別ポートで起動する構成**であるため、開発時およびテスト実行時にブラウザのクロスオリジン制約を受ける。§2.1のとおり認証はCookie(`HttpOnly`)方式であり、この制約が最も鋭く現れる。

* CORSヘッダを緩めるだけでは解決しない。ブラウザ仕様上、`Access-Control-Allow-Origin: *`(ワイルドカード)と `Access-Control-Allow-Credentials: true` は併用できず、ワイルドカードを用いると認証情報(Cookie)を伴うリクエストが拒否されるためである。
* **決定**: 開発・テスト時は、Vite開発サーバのproxy機能により `/api` へのリクエストをバックエンド(`http://localhost:8000`)へ転送し、**ブラウザから見て同一オリジンとして扱う**。バックエンド側にCORSミドルウェアを導入しない。フロントエンドのfetchはすべて相対パス(`/api/...`)とする。
* **理由**: (1) 同一オリジン化により、Cookieの `SameSite=Lax`(§2.1、`docs/P002-frontend-spec.md` §1で確定)をそのまま維持でき、開発用に `SameSite=None` へ緩める必要がない。(2) 相対パスfetchは、`docs/P302-deliver.md` が整備する本番配布トポロジー(リバースプロキシ配下で同一オリジンに集約する構成)とも一致するため、開発時と本番時で接続前提が変わらない。
* **検討した代替案**: バックエンドにCORSミドルウェアを導入し、オリジンを明示列挙したうえで `Access-Control-Allow-Credentials: true` を返す方式。却下。開発時のみ必要な設定が本番コードに残り、許可オリジンの管理が環境ごとに必要になるため。認証をトークン方式に変更する案も、ADR-005でCookie方式を採用済みであり、この制約のためだけに覆すのは影響が大きいため却下。
* この決定はADR-006とする。
* 本決定は、P103(結合テスト実行)・P201(受け入れ結合テスト実行)がテストハーネスを組む際、および `docs/P009-acceptance-direction.md` がテストの事前準備・実行コマンドを記載する際の前提となる。
