...
* **トポロジー**: クライアント・サーバ分離構成のテストハーネスは `docs/ADR.md` **ADR-006**(開発サーバのproxyによる同一オリジン化)に従う。本書および各T0NNテストで独自にトポロジーを決めない(`docs/P003-backend-spec.md` §2.3 参照)。
* **テストデータのライフサイクル**: `docs/P006-test-plan.md` §5「テストデータのライフサイクル方針」に従う。スイート実行ごとに1回、**バックエンド起動前**にE2E用データストアをベースライン(マイグレーション適用直後)へ復元する(`client/scripts/reset-e2e-db.mjs`、`client/playwright.config.ts` の `webServer` 起動コマンドから実行)。**各テストの「事前準備」に復元手順を書かない**(テストごとに復元すると、先行するテストが用意したデータまで消してしまうため)。
  * これにより、スイートを続けて何度実行しても同じ結果になる(スイート単位の再実行可能性)。個々のテストが単独で実行可能であることは保証しない。
  * 各テストの「事前準備」に書かれた「バックエンド・フロントエンドを起動する」は、Playwrightの `webServer` 設定による自動起動を指す。この起動の一部として上記の復元が行われる。
