...
* 仕様・実装・テスト・修正結果・配布資産を確認し、成果物一式をまとめる指示書 `docs/09-deliver.md` を作成する。
...
* `docs/06-impl-direction.md` (目次 OKF形式) および `docs/06-impl-direction/U000-{sprint-name}.md`
* `docs/07-test-direction.md` (目次 OKF形式) および `docs/07-test-direction/T000-{test-name}.md`
* `docs/test-records/*.md`
* `docs/08-fix-plan.md` (目次 OKF形式)、`docs/08-fix-plan/fixed/F000-{fix-name}.md`、`docs/08-fix-plan/08-fix-resolved.md`、`docs/08-fix-plan/08-fix-unresolved.md`
...
既に `docs/09-deliver.md` が存在する場合は、既存内容を読み、必要な差分を追記・更新する。無関係な記述を削除しない。

#### 実行前チェック

作業開始時に以下を確認する。

1. `docs/02-frontend-spec.md` と `docs/03-backend-spec.md` が存在するか
2. `docs/05-test-plan.md` が存在するか
3. `docs/test-records/*.md` (テスト実装またはテスト記録) が存在するか
4. `docs/08-fix-plan/08-fix-resolved.md` または `docs/08-fix-plan/08-fix-unresolved.md` が存在するか
5. ソースツリーからアプリケーション種別を推定できるか
6. `docs/09-deliver.md` が既に存在するか
...
|---|---|---|---|---|---|---|---|
| SPEC-001 | ... | docs/05-test-plan.md | docs/07-test-direction/T001-... | `npm run test:integration -- ...` | PASS | docs/test-records/... | OK |
```

仕様IDが既存文書にない場合は、見出しやAPI名・画面名・シナリオ名をもとに一時IDを付ける(例: `SPEC-API-001` `SPEC-SCREEN-001` `SPEC-FLOW-001` `SPEC-NFR-001`)。
...
* `docs/02-frontend-spec.md` `docs/03-backend-spec.md` のAPI、画面、データモデル、エラーケース、複雑な処理フローを確認する。
* `docs/05-test-plan.md` のテスト観点とテストレベルを確認する。
* `docs/07-test-direction.md` (目次) および `docs/07-test-direction/T000-{test-name}.md` のテストタスクIDを確認する。
* `docs/test-records/*.md` と `docs/08-fix-plan/08-fix-resolved.md` / `docs/08-fix-plan/08-fix-unresolved.md` の実行結果を確認する。
...
実装済みでない場合は、「10. 未整備事項」に記載する。最終仕上げとして軽微な追加で済む場合は実装してよいが、仕様変更を伴う場合は人間レビュー対象にする。
...
確認・整備対象: `Dockerfile` / `docker-compose.yml` または `compose.yml` / `.env.example` / `README.md` または `docs/09-deliver.md` の起動手順 / 必要な初期化スクリプト / DBマイグレーション手順 / ヘルスチェック / ログ出力先 / 永続ボリューム / ネットワーク設定。
...
`docs/09-deliver.md` には、実行環境で実行できるまでの手順を必ず書く。
...
`docs/09-deliver.md` には、最終確認として以下を記載する: Unit Test 実行コマンド / 結合テスト実行コマンド / 必要なE2E・システムテスト実行コマンド / テスト結果の格納先 / 合格条件。
...
以下がある場合は必ず記載する: 対応するテストがない仕様 / 実行結果がないテスト / 失敗したままのテスト(`docs/08-fix-plan/08-fix-unresolved.md` に記録が残っているもの) / 仕様矛盾 / Docker Compose 未整備 / CLI配布形式未整備 / バージョン確認手段がない / ビルド履歴がない / 実行手順が未確認 / 本番環境固有の値が未確認。
...
`docs/09-deliver.md` はワークフローの最終フェーズの成果物であり、後続フェーズはない。人間がリリース判断を行うための最終資料として参照する。

## 動作

* 共通指示に加えて、上記アウトプット記載内容(実行前チェック〜作業時の禁止事項)に従う。
* `docs/09-deliver.md` を作成または更新したら、そこで停止する。ユーザーには、作成・更新したファイル、アプリケーション種別、対応表の状態、バージョン/ビルド履歴の整備状況、配布資産の整備状況、実行手順の有無、リリース判定、未整備事項の有無を簡潔に報告する。
