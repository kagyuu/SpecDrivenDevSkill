...
* 既存のソースツリー(`app/` または `client/` `server/` など、`docs/P005-impl-plan.md` で定めたコード格納先)。実装着手前はこのディレクトリ自体がまだ存在しないことが普通にあるため、その場合は無いことを前提として扱う(後述「アウトプットの記載内容」参照)。
...
* **初期化済みだが未実装の場合**(本Skill自身のP007が必ず作るケース。ビルドツールによる `uv init`/`npm init` 等は済んでいるが、実装コードは0行)は、「コードベースが既にある」と「無い」のどちらの分岐にも単純には当てはまらない。この場合は、実在するファイル(`pyproject.toml`/`package.json` 等の初期化ファイル)を一覧に含めつつ、「(実装前)」の注記も残すハイブリッド形式で作成する(初期化済みファイルの情報を捨てず、かつ未実装であることも明示する)。
