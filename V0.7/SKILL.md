...
| P905 | `docs/ADR.md`(更新がある場合) または `docs/P903-cr-records/CR-NNN.md` への追記(更新不要の場合。標準はこちら) | ADRを更新した場合はその更新内容。更新不要と判断した場合は、その判定結果と理由の記録(「ADR更新: 不要(理由: ...)」の1行など) | |
...
* `docs/.inprogress` が存在せず、`docs/P001-requirement.md` 〜 `docs/P302-deliver.md` がすべて存在する場合、**直ちに「全フェーズ完了」と判定してはならない**。Refactor(P901〜P905)が進行中だった可能性を必ず確認する。
  ...
  * そこで、`docs/CR.md`(状態の台帳)の状態列を確認し、`対応中` または `反映確認中` のCRが残っていないかを見る。残っている場合は、そのCRについて **P904** から再開する。台帳は完了したCRの行も削除せず状態列だけを更新して残すため、この判定は常に台帳の全行を対象に行える。
