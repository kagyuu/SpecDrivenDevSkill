...
* その他: 変更要求(CR)にもとづいて設計書を修正した場合は、修正内容と修正理由をADR(P022)に追記する

#### WBS (PはPhaseの略)

* P002 ユーザインタフェース設計 (既存のP002、およびP001・P022・CRから作成/更新する)
* P003 システム詳細設計 (既存のP003、およびP001・P002・P022・CRから作成/更新する)
* P004 要求トレーサビリティマトリクス作成 (P001で定義された要求事項がP002・P003で満たされているかを検証する。満たされていなければP002に差し戻す)
* P005 実装計画 (既存のP005、およびP001・P002・P003・P004・P022・CRから作成/更新する)
* P006 テスト計画 (既存のP006、およびP001・P002・P003・P004・P005・P022・CRから作成/更新する)
...
* P020 実装構造生成/修正 (コードベースがあれば、その目次となる `{ソースツリー}/INDEX.md` を作る。無ければ空ファイルを作る。INDEX形式に準じる。主にAIが参照するが人間も見る)
* P021 ArchitectureHandbook.mdを作る (アプリケーションの技術的側面を明らかにする)
* P022 ADR.mdを整理する (仕様決定までの経緯を整理して、最終的にその仕様になった理由を簡潔に整理する。LLMが読み込める程度に小さくする。削除した経緯は別途ADR_master.mdに退避する)
...
| Overview Step | P021 | ArchitectureHandbook作成 |
| Overview Step | P022 | ADR整理 |
...
| P021 | `docs/ArchitectureHandbook.md` | アプリケーションの技術的側面のハンドブック | |
| P022 | `docs/ADR.md` / `docs/ADR_master.md` | 現在有効な設計判断(ADR) / 廃止済みADRの退避先 | |
...
| P902 | `docs/P001-requirement.md`(更新) + `docs/P001-requirement_{YYYYMMDD}.md`(原本退避) | 更新後のシステム要件定義書 / 更新前の原本 | |
...
* Markdown 内の図について
  ...
  * 図に関しては記述内容を評価しない
* 各フェーズで作成するドキュメントがない場合、あるいは不足する項目が多数にわたる場合は、Agent自身の想定にもとづいてドキュメントを作成し、末尾に "★FIXME★" を記載する。`docs/.inprogress` に、現在のフェーズ番号(P番号)を記入する。
* 各フェーズで作成するドキュメントがあり、かつ不足する項目が小数な場合は、ドキュメントの記述内容の十分性を評価する。
  * 記述不足箇所がある場合、Agent自身の想定で補完し、末尾に "★FIXME★" を記載する。
...
* すべてのフェーズ(P001〜P302)が完了しており、かつ `docs/CR.md` が新規/未反映の内容を含む → **P901 (Refactor)** から開始する
...
#### P021 — ArchitectureHandbook作成 (`docs/ArchitectureHandbook.md`)

`SKILL-P021-architecture-handbook.md` を実行する

#### P022 — ADR整理 (`docs/ADR.md`)

`SKILL-P022-adr.md` を実行する
