...
> `V0.7_testbed`(Skill V0.7のフルパイプライン実地検証)の起点となる要件定義書です。題材はV0.2_testbed以来の会議室予約システムを踏襲していますが、過去のテストヘッドでCR-001〜CR-003として追加した3項目(S03の終日チェックボックス、S06の説明文、予約の参加予定人数)は、本書では**最初から要件に含まれるもの**として記載しています。V0.7_testbedはP002以降をV0.7のルールで新規に実行するため、過去バージョンのCR履歴は引き継ぎません。
> (`docs/.inprogress` はP002に設定済みです)
>
> **改訂**: 2026-08-05 に CR-001(予約にオンライン会議URLを登録できるようにしてほしい)にもとづき P902 で更新しました。更新前の原本は `docs/P001-requirement_20260805_CR-001.md` に保持しています。CR-001による変更箇所には「※CR-001により追加」と注記しています。
...
| 表示 | 予約サマリ(予約者名・件名) | 予約済みセルに表示する簡易情報。参加予定人数は表示しない。※CR-001により、オンライン会議URLもこの画面には表示しない |
...
| 入力 | 終日チェックボックス | 任意。チェックすると開始時刻に09:00、終了時刻に18:00を自動入力する。自動入力後も開始時刻・終了時刻は手動で上書き可能。 |
| 入力 | 件名 | 必須、最大100文字 |
| 入力 | 参加者(社員) | 任意、複数選択 |
| 入力 | 参加予定人数 | 任意。1以上の整数。選択した会議室の収容人数を超える値はエラーとする。 |
| 入力 | オンライン会議URL | ※CR-001により追加。任意、最大500文字。入力する場合は `http://` または `https://` で始まっていなければエラーとする。空欄のままでも予約できる。 |
| 入力 | 備考 | 任意、最大500文字 |
| 表示 | 重複エラーメッセージ | 選択した会議室・時間帯が既に予約済みの場合に表示 |
| 表示 | 収容人数超過エラーメッセージ | 参加予定人数が選択した会議室の収容人数を超える場合に表示 |

### S04 予約詳細・編集画面

| 種別 | 項目名 | 内容 |
| --- | --- | --- |
| 表示 | 予約内容(会議室/日時/件名/参加者/参加予定人数/オンライン会議URL/備考/予約者) | S03と同項目を表示。※CR-001によりオンライン会議URLを追加 |
| 表示 | オンライン会議URLのリンク | ※CR-001により追加。登録されている場合はクリックできるリンクとして表示する。未登録の場合はリンクを表示しない |
| 入力 | 編集用の各項目 | 予約者本人または管理者のみ編集可。参加予定人数もS03と同じルールで編集可。※CR-001によりオンライン会議URLもS03と同じルールで編集可 |
...
| 種別 | 項目名 | 内容 |
| --- | --- | --- |
| 表示 | 会議室一覧(会議室名/収容人数/設備/説明文/有効・無効) | |
| 入力 | 会議室名・収容人数・設備・有効フラグ | 新規登録/編集時に入力 |
| 入力 | 説明文 | 任意入力、最大200文字。新規登録/編集時に入力 |
