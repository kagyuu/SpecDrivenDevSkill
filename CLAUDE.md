...
**V1.0 is the latest version and is committed** (`2b50f06 V1.0`, on top of `506fbec`/`96cf846 V1.0準備`). A follow-up change — the test re-runnability rule described below, plus its testbed fix — may still be uncommitted; check `git status`. V0.1–V0.7 were developed in Claude Cowork sessions running in a cloud sandbox; that workflow has since been fully handed off to Claude Code running locally, and V0.8 onward were developed locally without incident.

**No known open defects.** `V1.0_testbed` found two issues and both were resolved inside that round — see `V1.0_testbed/e2e-validation-report.md` §2-1 and §3-1. The larger of the two is worth knowing about because it shaped a new rule: the generated test suite was not re-runnable (fresh DB → 9/9 pass, immediate re-run against the same DB → 2 fail, because rows accumulate in a fixed-path SQLite file). That mattered because **P205 (結合テスト再実施) re-runs exactly those tests**, so it could report failures unrelated to the fix under test. The human's decision was to reset the data store to its baseline, accepting that individual tests are no longer independently runnable. See "Structural additions in V1.0" below for the resulting rule.

**Still pending as always: whether to create the next version, and what goes in it.** `V1.0_testbed/e2e-validation-report.md` §4 lists what the differential run could not cover (the P007→P102 initialization flow end to end, the `0.1.0` initial version on a fresh build, and the "cannot propose a fix" branch of the stop-report rule). Those are candidates for a future full-rebuild testbed, not defects. Do **not** start the next version on your own initiative — the standing rule is that the human decides each version's direction after reading the validation report. Summarize and ask; don't fix speculatively.
...
- **Test suites must be re-runnable, and that gets verified by actually running twice.** P006 decides the test-data lifecycle policy: at what granularity the store is restored to a baseline (default: once per suite run, *not* per test) and what "baseline" means (the state right after migrations, not an empty store — seed users usually come from a migration, so deleting all rows locks you out). P008/P009 carry that into each test's 事前準備, and `SKILL.md` requires whoever writes or changes tests (P103/P201/P203/P205) to run the suite twice and confirm the same result. The property being guaranteed is **suite-level** re-runnability, not per-test independence — tests may depend on each other's data if P006 says so. One hard-won detail is encoded in P008: the restore must happen **before the application under test starts**. A test framework's setup hook may run *after* it (Playwright's `globalSetup` starts after `webServer`), and restoring a store the app already has open destabilizes the run.
- **"Couldn't run" is a different diagnosis from "test failed."** `SKILL.md` requires that when a test run or build terminates without producing results (native crash, unusual exit code, no output), the agent suspect the environment/toolchain first, isolate with a **minimal reproduction** before concluding anything, and never leave it at "probably environmental" without that isolation. Environment findings get recorded in the project's `docs/ArchitectureHandbook.md` ch.9 (an input to P101, so it reaches later Executors). This rule exists because `V0.9_testbed` recorded a `vite build` crash as simply unexplained and carried it forward; the V1.0 round isolated it in two lines and found the real cause.

**Where environment-specific facts go — Skill vs. project.** A concrete fact like "`fs.rmSync` crashes on this machine's Node" must **not** go into the Skill: version-specific truths go stale, are false on other OSes, and the Skill gets copied to arbitrary machines (`SKILL-P007-impl-direction.md` forbids hardcoding stack names for the same reason). What goes into the Skill is the *procedure* — how to isolate and where to record. The fact itself belongs in the project's ArchitectureHandbook ch.9, and repo-level notes like this file. Apply this split when a future round turns up a similar environment quirk.
...
- **`fs.rmSync` crashes Node v24.12.0 on this machine** — natively, with `STATUS_STACK_BUFFER_OVERRUN` (exit `-1073740791`), whether or not the target exists. Use `fs.unlinkSync` in a try/catch ignoring `ENOENT` instead. This is almost certainly why `npm run build` (`vite build`) fails here regardless of source content, which `V0.9_testbed` had recorded as unexplained: vite cleans its outDir with `fs.rmSync`.

(`V1.0_testbed` no longer needs its DB deleted by hand — `client/scripts/reset-e2e-db.mjs` runs from the `webServer` command and resets it per suite run.)
