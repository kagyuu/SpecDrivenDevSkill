...
from app.core.validators import validate_capacity, validate_room_description, validate_room_name
...
def create(conn: sqlite3.Connection, data: dict) -> sqlite3.Row:
    ...
    if not validate_room_description(data.get("description")):
        # CR-002で追加。
        raise ValidationError("説明文は200文字以内で入力してください",
                               details=[{"field": "description", "reason": "too long"}])
...
def update(conn: sqlite3.Connection, room_id: int, data: dict) -> sqlite3.Row:
    ...
    equipment = data.get("equipment") or []
    equipment_str = ",".join(equipment) if isinstance(equipment, list) else str(equipment)
    # F004修正: description をrepository呼び出しに渡し忘れていたため常にNoneで上書きされていた。
