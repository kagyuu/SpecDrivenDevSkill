...
> `V0.9_testbed`(Skill V0.9のフルパイプライン実地検証)の起点となる要件定義書です。`V0.7_testbed`/`V0.8_testbed`と同じ起点要件定義書(CR-001適用前の原本)をそのまま流用し、以降のP002〜P302をV0.9のルールで新規に実行します。要件文書そのものはV0.9で変更していないため、過去バージョンとの差分がSkillのルール変更に起因するものだけになるようにしています。
...
| 入力 | 備考 | 任意、最大500文字 |
| 入力 | 備考(社内向けメモ) | 任意、最大300文字。予約の作成者本人・管理者のみ閲覧可能(他の利用者には一切表示しない)。※CR-001により追加 |
...
| 種別 | 項目名 | 内容 |
| --- | --- | --- |
| 表示 | 予約内容(会議室/日時/件名/参加者/参加予定人数/備考/予約者) | S03と同項目を表示 |
| 表示・入力 | 備考(社内向けメモ) | 予約の作成者本人・管理者のみ表示・編集可能。任意、最大300文字。※CR-001により追加 |
| 入力 | 編集用の各項目 | 予約者本人または管理者のみ編集可。参加予定人数もS03と同じルールで編集可 |
