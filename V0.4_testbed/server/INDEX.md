...
> P104(実装構造修正)で更新。会議室予約システムのバックエンド(全4スプリント U001〜U004実装済み)。
...
* テスト: `unittest`(標準ライブラリ。pytestが利用不可のための代替)。`server/tests/` に121件、全PASS。
...
* `app/db/`
  * `connection.py` — SQLite接続取得(`get_connection`)・マイグレーション適用(`init_db`)。
...
* `app/core/`
  ...
  * `validators.py` — 社員ID/パスワード/会議室名/収容人数/氏名/権限/時間範囲/件名/備考の各バリデーション純粋関数。
...
* `app/api/` — Starletteルーター(FastAPI Handler層の代替)。
  ...
  * `rooms.py` — `GET/POST /api/rooms`、`PUT/DELETE /api/rooms/{room_id}`。
...
* 単体テスト全件: `cd server && python3 -m unittest discover -s tests -v`(121件PASS)。
