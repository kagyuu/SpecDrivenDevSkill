...
# 【修正タスクID】F0xx(`docs/08-fix-plan/F000-{fix-name}.md` のファイル名・目次のIDと一致させる)

## 【対応する失敗テスト】T0xx(このF0xxが修正する、`docs/07-test-direction/T000-{test-name}.md` の失敗したテストID)
...
* 修正しきれなかった障害が1件でもある場合、`TEMPLATE-08-fix-unresolved.md` の構成に従って `docs/08-fix-plan/08-fix-unresolved.md` を作成または更新するよう指示する。
