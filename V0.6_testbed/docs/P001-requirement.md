...
> 本書は `spec-driven-dev` Skill フェーズP001の成果物です。
> `V0.2_testbed`(Skill V0.2でのフルパイプライン実地検証)の `docs/01-requirement.md` を、V0.4のドキュメント命名規則(`docs/P0NN-*.md`)およびフェーズ番号(P番号)に合わせて移植したものです。要件の内容自体はV0.2_testbedから変更していません。
> (`docs/.inprogress` はP002に設定済みです)
> **2026-08-03: CR-001(S03に「終日」チェックボックスを追加)にもとづき、P902で本書を更新しました。更新前の原本は `docs/P001-requirement_20260803.md` に保持しています。変更箇所には「※CR-001により追加」と注記しています。**
> **2026-08-03: CR-002(S06に説明文(description)を追加)にもとづき、P902で本書を更新しました。更新前の原本は `docs/P001-requirement_20260803_CR-002.md` に保持しています(V0.5 defect 2-9対応、CR番号をファイル名に含める命名規則)。変更箇所には「※CR-002により追加」と注記しています。**
> **2026-08-04: CR-003(予約に参加予定人数(attendee_count)を追加)にもとづき、P902で本書を更新しました。更新前の原本は `docs/P001-requirement_20260804_CR-003.md` に保持しています。変更箇所には「※CR-003により追加」と注記しています。**
...
| 表示 | 予約サマリ(予約者名・件名) | 予約済みセルに表示する簡易情報。参加予定人数は表示しない(※CR-003により明記) |
...
| 入力 | 終日チェックボックス | 任意。チェックすると開始時刻に09:00、終了時刻に18:00を自動入力する。自動入力後も開始時刻・終了時刻は手動で上書き可能(※CR-001により追加)。 |
| 入力 | 件名 | 必須、最大100文字 |
| 入力 | 参加者(社員) | 任意、複数選択 |
| 入力 | 参加予定人数 | 任意。1以上の整数。選択した会議室の収容人数を超える値はエラーとする(※CR-003により追加)。 |
| 入力 | 備考 | 任意、最大500文字 |
| 表示 | 重複エラーメッセージ | 選択した会議室・時間帯が既に予約済みの場合に表示 |
| 表示 | 収容人数超過エラーメッセージ | 参加予定人数が選択した会議室の収容人数を超える場合に表示(※CR-003により追加) |

### S04 予約詳細・編集画面

| 種別 | 項目名 | 内容 |
| --- | --- | --- |
| 表示 | 予約内容(会議室/日時/件名/参加者/参加予定人数/備考/予約者) | S03と同項目を表示(※参加予定人数はCR-003により追加) |
| 入力 | 編集用の各項目 | 予約者本人または管理者のみ編集可。参加予定人数もS03と同じルールで編集可(※CR-003により追加) |
...
| 種別 | 項目名 | 内容 |
| --- | --- | --- |
| 表示 | 会議室一覧(会議室名/収容人数/設備/説明文/有効・無効) | ※説明文はCR-002により追加 |
| 入力 | 会議室名・収容人数・設備・有効フラグ | 新規登録/編集時に入力 |
| 入力 | 説明文 | 任意入力、最大200文字。新規登録/編集時に入力(※CR-002により追加) |
...
認証方式(セッション/JWT等)や重複チェックの厳密な仕様(同時リクエスト時の排他制御など)は、次フェーズ(バックエンド詳細設計、`docs/P003-backend-spec.md`)で確定する。
...
* 詳細なテスト計画(テスト項目・観点・スケジュール)はP006(`docs/P006-test-plan.md`)で定める。本フェーズでは方針レベルに留める。
