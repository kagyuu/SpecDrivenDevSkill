...
- [x] U007 [cr-003-attendee-count: 予約の参加予定人数](./P007-impl-direction/U007-cr-003-attendee-count.md) — CR-003対応。S03・S04に参加予定人数(attendee_count)を追加し、`reservations`テーブル・POST/PUT/GET reservations API・収容人数チェックに反映する。あわせてマイグレーションを差分適用方式に切り替える

## 進捗更新ルール

* 各スプリント(U001〜U004)の実装・単体テストがすべて完了したら、ExecutorはP102の一部として該当行のチェックボックスを `[x]` に更新する。
* 全スプリントが `[x]` になるまでP007(実行対象スプリント群)は完了しない。
* U005・U006・U007は、通常のスプリントではなくRefactor(P901〜P905)のP903でCR-001・CR-002・CR-003対応としてそれぞれ追加したものである。実装・単体テストの完了後に `[x]` へ更新する点は他スプリントと同じ扱いとする。
