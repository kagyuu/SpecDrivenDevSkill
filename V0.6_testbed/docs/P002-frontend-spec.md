...
| 予約サマリ | 表示専用。予約者氏名・件名を表示する。他人の予約も含め全予約者に表示する(社内共有カレンダーのため)。参加予定人数(`attendee_count`)は表示しない(※CR-003により明記。`GET /api/reservations` のレスポンスにも含めない)。 |
...
| 参加予定人数 | 任意。1以上の整数(未入力=`null` を許容する)。選択した会議室の収容人数(`ROOMS.capacity`)以下であること。超過した場合はサーバーが `400 VALIDATION_ERROR` を返す(※CR-003により追加)。★FIXME★ 参加者(社員)の選択人数と参加予定人数を一致させる必要があるかはCR・P001に指定が無いため、両者は独立した項目(参加予定人数は社外参加者・未確定の参加者も含みうる概算値)と仮定した。 |
| 備考 | 任意。最大500文字。 |
| 重複エラーメッセージ | サーバーから `RESERVATION_CONFLICT`(409)を受けたとき「選択した会議室・時間帯は既に予約されています」を表示する。 |
| 収容人数超過エラーメッセージ | サーバーから `VALIDATION_ERROR`(400、`field=attendee_count`)を受けたとき「参加予定人数が会議室の収容人数を超えています」を表示する(※CR-003により追加)。 |

### S04 予約詳細・編集画面

| 項目 | ルール |
| --- | --- |
| 予約内容表示 | S03と同一項目(会議室/日付/時刻/件名/参加者/参加予定人数/備考)に加え、予約者(作成者)氏名を表示する(※参加予定人数はCR-003により追加)。 |
...
| 説明文 | 任意。最大200文字。一覧表示・登録フォーム・編集フォームいずれにも表示する(※CR-002により追加)。★FIXME★ 改行やMarkdown等の書式をサポートするかP001に指定がないため、プレーンテキスト(改行はそのまま保持、装飾なし)と仮定した。 |
| 削除ボタン | 論理削除(有効フラグをOFFにする)。物理削除はしない(P001に明記)。無効化した会議室が既存の未来予約を持つ場合でも、既存予約は残す(新規予約の選択肢からのみ除外する)。★FIXME★ 既存予約の扱いがP001に指定がないため仮定した。 |

> **2026-08-03: CR-002(S06に説明文(description)を追加)にもとづき、本節および§4.4〜§4.6を更新しました。S03(予約作成画面)の会議室プルダウンはスコープ外のため変更していません(`docs/CR.md` CR-002「影響が想定される範囲」参照)。**
>
> **2026-08-04: CR-003(予約に参加予定人数(attendee_count)を追加)にもとづき、§3 S02/S03/S04、§4.8〜§4.12、§5.1・§5.2を更新しました。S02(予約カレンダー画面)の予約サマリおよび `GET /api/reservations` のレスポンスは、CRの指定によりスコープ外のため変更していません(`docs/cr-records/CR-003.md`「影響が想定される範囲」参照)。**
...
{
  "rooms": [
    { "id": 1, "name": "会議室A", "capacity": 6, "equipment": ["プロジェクタ"], "is_active": true, "description": "役員会議専用の個室です。" }
  ]
}
```

* `description`(※CR-002により追加): 任意項目。未設定の場合は `null`。
* エラー: `401 AUTH_REQUIRED`

### 4.5 POST /api/rooms

* 認証: 必須 / 権限: 管理者
* リクエストボディ: `{ "name": "会議室A", "capacity": 6, "equipment": ["プロジェクタ"], "is_active": true, "description": "役員会議専用の個室です。" }`(`description` は任意項目、※CR-002により追加。未指定または `null` で登録可能)
* レスポンス 201: 作成された会議室オブジェクト(4.4のroom要素と同形式、`description` を含む)
* エラー: `400 VALIDATION_ERROR`(`description` が200文字を超える場合を含む、※CR-002により追加)、`401 AUTH_REQUIRED`、`403 FORBIDDEN`、`409`(同名の有効な会議室が既存の場合。コード `VALIDATION_ERROR` を流用し `details` に `field: name` を設定する)★FIXME★ 専用エラーコードを設けるか流用するかは実装判断に委ねる。

### 4.6 PUT /api/rooms/{room_id}

* 認証: 必須 / 権限: 管理者
* リクエストボディ: 4.5と同形式(部分更新ではなく全項目指定の全量更新とする)★FIXME★ PATCH的な部分更新かP001に指定がないため全量更新と仮定した。`description` を変更しない場合も、既存値をそのまま含めて送信すること(全量更新のため。※CR-002により追記)。
* レスポンス 200: 更新後の会議室オブジェクト(`description` を含む)
...
```
  ...
  * 一覧表示用のため、備考・参加者一覧・参加予定人数(`attendee_count`)はここでは返さない(詳細は4.10で取得する)。参加予定人数を返さないのは、S02の予約サマリに表示しないというCR-003の指定によるものである(※CR-003により明記。本APIのレスポンス形式自体はCR-003で変更しない)。★FIXME★ カレンダー表示に参加者名まで必要かP001に明記がないため、簡易情報(予約者名・件名)のみとした(P001「予約サマリ」欄の記述と整合)。
...
* レスポンス 200: 4.8と同じreservation要素に `attendee_count`(整数または `null`)を加えた配列(自分が作成した予約のみ)(※CR-003により追加)
...
```json
{
  "id": 10, "room_id": 1, "date": "2026-08-10",
  "start_time": "10:00", "end_time": "11:00",
  "title": "定例MTG", "notes": "資料は事前配布", "attendee_count": 8,
  "participants": [{ "id": 2, "name": "鈴木花子" }],
  "created_by": { "id": 1, "name": "山田太郎" }
}
```

* `attendee_count`: 参加予定人数。未設定の場合は `null`(※CR-003により追加)。
...
* `attendee_count`(※CR-003により追加): 任意。省略・`null` を許容する。指定する場合は1以上の整数で、かつ `room_id` で指定した会議室の `capacity` 以下であること。`capacity` と等しい値は許容する。違反時は `400 VALIDATION_ERROR`(`details[].field = "attendee_count"`)。
* エラー: `400 VALIDATION_ERROR`(参加予定人数が不正・収容人数超過を含む)、`401 AUTH_REQUIRED`、`404 NOT_FOUND`(存在しない会議室・参加者ID)、`409 RESERVATION_CONFLICT`(同一会議室・重複時間帯の既存予約あり)
...
* リクエストボディ: 4.11と同形式(全量更新。`attendee_count` を含む。全量更新のため、省略した場合は `null` で上書きされる)(※CR-003により追加)
* レスポンス 200: 更新後のオブジェクト(`attendee_count` を含む)
* 重複チェック: 自分自身の予約は重複判定から除外したうえで、他の予約との重複を判定する。
* エラー: `400 VALIDATION_ERROR`(参加予定人数が不正・収容人数超過を含む。収容人数の判定は、更新後の `room_id` が指す会議室の `capacity` に対して行う)、`401 AUTH_REQUIRED`、`403 FORBIDDEN`(予約者本人でも管理者でもない)、`404 NOT_FOUND`、`409 RESERVATION_CONFLICT`
...
| attendee_count | INTEGER | NULL可 | 参加予定人数。1以上の整数。予約先会議室の `ROOMS.capacity` 以下(※CR-003により追加) |
...
* 参加予定人数(`attendee_count`、CR-003)と参加者(社員)選択リストの人数を一致させるかどうかがCR・P001に明記されていない。本書では独立した項目と仮定した(§3 S03の★FIXME★参照)。
