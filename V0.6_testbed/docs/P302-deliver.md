...
## 12. CR対応時の再実行記録(P903内でのP302再実行)

### 12.1 CR-001・CR-002

* いずれもP903のスコープ決定で **P302は対象外** と判断した(納品物・配布資産・起動手順そのものに変更が生じないため)。したがって本書§8・§9のテスト件数は、P302を最初に実行した2026-08-03時点(バックエンド121件・フロントエンド35件・合計186件)の記録のまま据え置かれている。

### 12.2 CR-003(2026-08-04、予約の参加予定人数)— **P302は対象**

CR-003はデータモデル変更を伴い、マイグレーションの適用方式そのものを変更した(`docs/P003-backend-spec.md` §6.4)。配布物は繰り返し起動されるため、この変更は納品物の起動可否に直接影響する。したがってP903のスコープ決定でP302を対象に含め、「実行前チェック」を再実行した。

**実行前チェックの再実行結果(`SKILL-P302-deliver.md`「実行前チェック」1〜8)**

| 項目 | 結果 |
|---|---|
| 1. P002/P003の存在 | OK(いずれもCR-003で更新済み) |
| 2. P006の存在 | OK(CR-003で「運用(再起動耐性)」観点を追加) |
| 3. テスト記録の存在 | OK(`docs/test-records/20260804-1520-cr003-test-record.md` を追加) |
| 4. P202解決済み/未解決一覧の存在 | OK(CR-003では新規のF項目は発生していない) |
| 5. アプリケーション種別の推定 | OK(サービス提供型システム、変更なし) |
| 6. `docs/P302-deliver.md` の存在 | OK(本書。差分更新で対応) |
| 7. フロントエンドのAPI接続前提と配布トポロジーの整合 | OK(下記12.3の検証で確認) |
| 8. 再起動時の起動可否(初期化処理の冪等性) | OK(下記12.4の検証で確認) |

### 12.3 項目7(デプロイトポロジー整合性)の検証

本書§10に既記載のとおり、`client/js/api/*.js` は `BASE_URL=""` で `/api/...` への相対パスfetchを行う(同一オリジン前提)。現行の配布トポロジーは `nginx.conf` によるリバースプロキシで client(静的配信)と server(API)を単一オリジンに統合しており、前提と一致している。

**本ラウンドでは、この確認手順が「実際に壊れた状態から不一致を検出できるか」を検証するため、意図的にトポロジーを壊した状態を一時的に作って項目7を適用した**(V0.4 defect 1-6の動的検証。詳細と結果は `e2e-validation-report.md` を参照)。

* 壊した内容(一時的・検証目的): `nginx.conf` を削除し、`docker-compose.yml` から nginx サービスを外して server を `8000:8000`、client を `8080:8080` の別オリジンで公開する構成に置き換えた。
* 項目7を適用した結果、「client/server を別コンテナで配布する構成」に該当し、かつフロントエンドが同一オリジン相対パスを前提としているため、**不一致として検出できた**。
* 実測でも確認した。client オリジン(`http://127.0.0.1:8080/api/me`)への要求は静的配信サーバーが `404` を返し(=SPAのAPI呼び出しが一切通らない)、一方で server オリジン(`http://127.0.0.1:8000/api/me`)への直接要求は `401 AUTH_REQUIRED` を返した(=P009受け入れテストの接続方法では正常に見える)。この非対称性が、まさに項目7が防ごうとしている検出漏れである。
* 「判定」の規定(「これを不具合として扱い、本フェーズを完了とする前に…修正する。単に『10. 未整備事項』に記載するだけで済ませ、壊れたトポロジーを黙って納品しない」)に従い、**本フェーズを完了とする前に元の単一オリジン構成(`nginx.conf` + nginx サービス)へ復旧した**。復旧後の `docker-compose.yml`・`nginx.conf` は破壊前と同一(md5一致)であることを確認済みである。

### 12.4 項目8(再起動時の起動可否)の検証

* CR-003で追加した `0003_add_reservation_attendee_count.sql` は `ALTER TABLE ... ADD COLUMN` を含む非冪等なDDLである。
* 判定の規定に従い、**実際に初期化処理を2回連続で実行して確認した**。同一のDBファイルに対して `init_db()` を1回目・2回目・3回目と実行し、いずれも成功すること、`SCHEMA_MIGRATIONS` に3件のマイグレーションが記録されること、スキーマ・既存データが壊れないことを確認した(実測ログは `docs/test-records/20260804-1520-cr003-test-record.md` T028)。
* さらに、2回のマイグレーションを経たDBファイルに対して実際に `uvicorn` を起動し、`GET /api/me` が `401 AUTH_REQUIRED` を返す(=アプリが正常に起動して応答している)ことを確認した。
* 自動化版として `server/tests/test_db_init.py::DbInitIdempotencyTest`(4件)を追加し、以後の回帰でも確認される状態にした。

### 12.5 CR-003適用後のテスト件数

* Unit Test(バックエンド): `cd server && python3 -m unittest discover -s tests -v` → **159件**、全PASS(P302初回作成時121件 → CR-002で134件 → CR-003で159件)。
* Unit Test(フロントエンド): `cd client && node --test tests/*.test.mjs` → **57件**、全PASS(P302初回作成時35件 → CR-001で38件 → CR-002で45件 → CR-003で57件)。
* 結合テスト(P008): T001〜T028 の **28件**、全PASS(CR-002でT021〜T023、CR-003でT024〜T028を追加)。
* 受け入れ結合テスト(P009): A001〜A010 の **10件**(CR-003はスコープ外のため再実行せず、件数・内容とも変更なし)。

### 12.6 リリース判定への影響

* §11の判定(**保留**)は変更しない。CR-003により配布資産の未検証状態(Dockerデーモン不在によるビルド・起動未確認)・TLS終端未整備・初期管理者作成手段の未整備という保留理由が解消されたわけではないため。
* ただし、保留理由のうち「初回起動でしか成功しない初期化処理」というリスクについては、CR-003で差分適用方式に切り替えたことで解消している。
