...
> P104(実装構造修正)で更新。会議室予約システムのバックエンド(全4スプリント U001〜U004実装済み)。CR-002対応(U006、会議室説明文)・CR-003対応(U007、予約の参加予定人数)で追加更新。
...
* テスト: `unittest`(標準ライブラリ。pytestが利用不可のための代替)。`server/tests/` に159件、全PASS(CR-002対応で121件→134件、CR-003対応で134件→159件)。
...
* `app/db/`
  * `connection.py` — SQLite接続取得(`get_connection`)・マイグレーション適用(`init_db`)。CR-003対応で、`SCHEMA_MIGRATIONS` テーブル(適用済みファイル名の記録)による**差分適用方式**に変更した(適用方式の定義は `docs/P003-backend-spec.md` §6.4)。CR-002時点の「`duplicate column name` を捕捉してファイルの残りを読み飛ばす」実装は、後続の文が黙って適用されない問題があるため廃止した。`applied_migrations(conn)` で適用済み一覧を確認できる。
  ...
  * `migrations/0003_add_reservation_attendee_count.sql` — CR-003対応。`RESERVATIONS.attendee_count`(INTEGER、NULL可)を追加。
* `app/core/`
  ...
  * `validators.py` — 社員ID/パスワード/会議室名/収容人数/氏名/権限/時間範囲/件名/備考の各バリデーション純粋関数。`validate_room_description`(CR-002対応で追加、任意・最大200文字)。`validate_attendee_count`(CR-003対応で追加、任意・1以上の整数)。
...
* `app/services/` — 業務ロジック層。`auth_service.py`(ログイン/ログアウト/セッション解決)、`room_service.py`、`user_service.py`(自己無効化禁止含む)、`reservation_service.py`(会議室/参加者存在確認、権限チェック`check_editable`、CR-003の参加予定人数と会議室収容人数の突き合わせを含む)。
* `app/api/` — Starletteルーター(FastAPI Handler層の代替)。
  ...
  * `reservations.py` — `GET/POST /api/reservations`、`GET /api/reservations/mine`、`GET/PUT/DELETE /api/reservations/{reservation_id}`。全17エンドポイント実装済み。CR-003対応で `GET /api/reservations/mine` のレスポンスに `attendee_count` を追加(`GET /api/reservations` には追加しない。S02の予約サマリに表示しないため)。
...
* 単体テスト全件: `cd server && python3 -m unittest discover -s tests -v`(159件PASS)。
* 再起動耐性(初期化処理の冪等性)の確認: 同一のDBファイルに対して `DATABASE_PATH=<file> python3 -c "from app.db.connection import init_db; init_db()"` を2回連続で実行し、2回目も成功することを確認する(`docs/P006-test-plan.md` §3 運用(再起動耐性)、`docs/P302-deliver.md` 実行前チェック項目8)。自動化版は `python3 -m unittest tests.test_db_init.DbInitIdempotencyTest`。
