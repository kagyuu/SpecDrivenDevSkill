...
class ReservationValidatorTest(unittest.TestCase):
    ...
    # --- CR-003(U007-1): 参加予定人数 ---

    def test_attendee_count_none_allowed(self):
        self.assertTrue(validate_attendee_count(None))

    def test_attendee_count_positive_integer_allowed(self):
        self.assertTrue(validate_attendee_count(1))
        self.assertTrue(validate_attendee_count(100))

    def test_attendee_count_zero_or_negative_rejected(self):
        self.assertFalse(validate_attendee_count(0))
        self.assertFalse(validate_attendee_count(-1))

    def test_attendee_count_non_integer_rejected(self):
        self.assertFalse(validate_attendee_count("8"))
        self.assertFalse(validate_attendee_count(1.5))

    def test_attendee_count_bool_rejected(self):
        # bool は int のサブクラスだが、人数としては不正として扱う
        self.assertFalse(validate_attendee_count(True))
