...
U007-1: attendee_count の永続化・読み出し (CR-003)
"""
...
def insert_with_participants(conn: sqlite3.Connection, data: dict, current_user_id: int) -> int:
    ...
    try:
        ...
        now = _now_iso()
        cur = conn.execute(
            "INSERT INTO RESERVATIONS (room_id, date, start_time, end_time, title, notes, "
            "attendee_count, created_by, created_at, updated_at) "
            "VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
            (data["room_id"], data["date"], data["start_time"], data["end_time"], data["title"],
             data.get("notes"), data.get("attendee_count"), current_user_id, now, now),
        )
...
def find_with_detail(conn: sqlite3.Connection, reservation_id: int) -> dict | None:
    ...
    return {
        ...
        "title": row["title"],
        "notes": row["notes"],
        "attendee_count": row["attendee_count"],  # CR-003: 参加予定人数(未設定はNone)
...
def update_with_participants(conn: sqlite3.Connection, reservation_id: int, data: dict) -> None:
    ...
    try:
        ...
        conn.execute(
            ...
            "notes = ?, attendee_count = ?, updated_at = ? WHERE id = ?",
            (data["room_id"], data["date"], data["start_time"], data["end_time"], data["title"],
             data.get("notes"), data.get("attendee_count"), now, reservation_id),
        )
