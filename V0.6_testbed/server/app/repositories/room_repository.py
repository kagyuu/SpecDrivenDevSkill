...
def insert(conn: sqlite3.Connection, name: str, capacity: int, equipment: str,
           is_active: bool = True, description: str | None = None) -> sqlite3.Row:
    # description: CR-002で追加(任意入力、最大200文字。検証はservice層で実施)。
    now = _now_iso()
    cur = conn.execute(
        "INSERT INTO ROOMS (name, capacity, equipment, is_active, description, created_at, updated_at) "
        "VALUES (?, ?, ?, ?, ?, ?, ?)",
        (name, capacity, equipment, 1 if is_active else 0, description, now, now),
    )
    conn.commit()
...
def update(conn: sqlite3.Connection, room_id: int, name: str, capacity: int, equipment: str,
           is_active: bool, description: str | None = None) -> sqlite3.Row | None:
    # description: CR-002で追加。呼び出し側(room_service.update)が省略した場合はNoneで上書きする。
    if find(conn, room_id) is None:
        return None
    now = _now_iso()
    conn.execute(
        "UPDATE ROOMS SET name = ?, capacity = ?, equipment = ?, is_active = ?, description = ?, "
        "updated_at = ? WHERE id = ?",
        (name, capacity, equipment, 1 if is_active else 0, description, now, room_id),
    )
    conn.commit()
