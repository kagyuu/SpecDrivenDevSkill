...
U007-1: validate_attendee_count (CR-003)
"""
...
def validate_room_description(value) -> bool:
    """任意入力、最大200文字 (docs/P002-frontend-spec.md §3 S06、CR-002で追加)。

    未入力(None)・空文字列はいずれも許容する(任意項目のため)。
    """
    if value is None:
        return True
...
def validate_attendee_count(value) -> bool:
    """参加予定人数。任意入力、1以上の整数 (docs/P002-frontend-spec.md §3 S03、CR-003で追加)。

    未入力(None)は許容する。会議室の収容人数(ROOMS.capacity)との比較は、
    予約と会議室の2エンティティにまたがるため純粋関数にできず、Service層で行う
    (docs/P003-backend-spec.md §7.11 の設計判断を参照)。
    """
    if value is None:
        return True
    if isinstance(value, bool):  # bool は int のサブクラスなので明示的に除外する
        return False
